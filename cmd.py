    ...
    def process(self, inputs, cfg):
        ...
        try:
            if inputs[0] == "upload":
                # upload --src=* --save=*
                assert len(inputs) == 5
                cfg.cmd_type = self.COMMAND.upload
                ...
                cfg.cmd_type = self.COMMAND.read
                for i in range((len(inputs) - 1) // 2):
                    command = inputs[i * 2 + 1]
                    param = inputs[i * 2 + 2]
                    if command == "--id":
                ...
                # fetch --path=* --id=*
                assert len(inputs) == 5
                cfg.cmd_type = self.COMMAND.fetch
                for i in range((len(inputs) - 1) // 2):
                    command = inputs[i * 2 + 1]
                    param = inputs[i * 2 + 2]
                    if command == "--path":
                ...
                assert len(inputs) == 0
                cfg.cmd_type = self.COMMAND.quit
            elif inputs[0] == "mkdir":
                # mkdir --dir=*
                assert len(inputs) == 3
                cfg.cmd_type = self.COMMAND.mkdir
                assert inputs[1] == "--dir"
                cfg.file_dir = inputs[2]
            elif inputs[0] == "help":
                assert len(inputs) == 0
                cfg.cmd_type = self.COMMAND.help
