...
class DataSever(threading.Thread):
    ...
    def run(self):
        config = self.config
        while True:
            config.data_events[self.server_id].wait()
            if config.cmd_flag:
                if config.cmd_type == self.config.COMMAND.upload and self.server_id in config.server_chunk_map:
                    self.save()
                elif config.cmd_type == self.config.COMMAND.read:
                    self.read()
                else:
                    pass
            config.data_events[self.server_id].clear()
            config.main_events[self.server_id].set()

    def save(self):
        """Data Sever save file"""
        data_node_dir = self.config.DATA_NODE_DIR % (self.server_id,)
        with open(self.config.file_path, 'r') as f_in:
            for chunk, offset, count in self.config.server_chunk_map[self.server_id]:
                f_in.seek(offset, 0)
    ...
    def read(self):
        ...
        read_path = (self.config.DATA_NODE_DIR % (self.server_id,)) + os.path.sep + self.config.read_chunk
